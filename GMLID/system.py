...
class LensSystem:
    ...
    def recalculate_com(self):
        ...
        if mass_sum == 0.0:
            ...
            return
        self._lens_mass = max(lens.m for lens in self.lenses)
        self._lens_com = x_sum / mass_sum, y_sum / mass_sum

    def recalculate_einstein_radius(self):
        self._einstein_angle = calculate_einstein_angle(
            self._lens_mass, self._lens_distance, self._source_distance
        )
    ...
    def pack_lenses(self) -> Generator[float, None, None]:
        ...
        for lens in self._lenses:
            yield lens.m
            yield (
                calculate_einstein_angle(lens.m, self._lens_distance, self.source_distance) / e
            ) ** 2
...
class LensImage:
    ...
    def update(self):
        ...
        self._lens_block.write(
            pack(f"2i {len(self._system.lenses) * 4}f", count, 0, *self._system.pack_lenses())
        )
