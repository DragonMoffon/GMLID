...
class DebugHistogramView(View):
    ...
    def __init__(self):
        ...
        self.histogram_geometry = ctx.geometry(
            [gl.BufferDescription(ctx.buffer(data=j.tobytes()), "2f", ["origin"])],
            mode=gl.POINTS
        )
        self.histogram_program = ctx.load_program(
            vertex_shader=get_glsl("IRS_histogram_vs"),
            fragment_shader=get_glsl("IRS_histogram_fs")
        )

        self.system = LensSystem(4000, 8000)
        self.system.extend_lenses((
            Lens(0.0, 0.0, 1.0), # Sun
            Lens(5.2, 0.0, 1.0), # Test Mass
