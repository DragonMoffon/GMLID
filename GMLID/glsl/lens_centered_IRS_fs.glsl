...
vec2 find_deflection(vec2 ray, vec2 lens, float radius_sqr){
  ...
  // we don't need the sqrt because we need to normalise relative and also divide by seperation which is sqrt * sqrt.
  float seperation = dot(relative, relative);
  return radius_sqr * relative / seperation;
}
